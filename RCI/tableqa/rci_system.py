...
class TableQAOptions:
    def __init__(self):
        ...
        # self.model_type = "albert"
        # self.tokenizer = "/data1/zrj/row-column-intersection/albert-base-v2"
        # self.row_model = (
        #     "/data1/zrj/row-column-intersection/datasets/wtq/models/row_alb"
        # )
        # self.col_model = (
        #     "/data1/zrj/row-column-intersection/datasets/wtq/models/col_alb"
        # )
        self.model_type = "bert"
        self.tokenizer = "/data1/zrj/row-column-intersection/bert-base-chinese"
        self.row_model = (
            "/data1/zrj/row-column-intersection/datasets/IM-TQA/models/row_bert_5"
        )
        self.col_model = (
            "/data1/zrj/row-column-intersection/datasets/IM-TQA/models/col_bert_5"
        )
...
class RCISystem(object):
    ...
    def row_column_strings(
        ...
        for row in rows:
            row_rep = " * ".join(
                [h + " : " + str(c) for h, c in zip(header, row) if c]
            )  # for sparse table use case
    ...
    def _repr_to_input(self, query: str, reprs: List[str]) -> Dict[str, torch.Tensor]:
        inputs = self.tokenizer.batch_encode_plus(
            batch_text_or_text_pairs=[(query, rr) for rr in reprs],
    ...
    def _top_k_cells(
        self, row_logits: np.ndarray, col_logits: np.ndarray
    ...
    def _apply(
        self, query: str, header: List[str], rows: List[List[str]]
    ) -> List[Tuple[int, int, float]]:
    ...
    def get_answer_columns(
        self, question: str, header: List[str], rows: List[List[str]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        return [
            {"col_ndx": ci, "confidence_score": s}
            for ci, s in all_scores[0 : self.opts.top_k]
        ]
    ...
    def get_answer_rows(
        ...
        all_scores.sort(key=lambda x: x[1], reverse=True)
        return [
            {"row_ndx": ri, "confidence_score": s}
            for ri, s in all_scores[0 : self.opts.top_k]
        ]
    ...
    def get_answers(
        ...
        return [
            {"row_ndx": ri, "col_ndx": ci, "confidence_score": s, "text": rows[ri][ci]}
            for ri, ci, s in cells
        ]

    def get_answers_with_reps(self, query, rows, row_reps, col_reps):
        col_logits = self._batched_score(query, col_reps, self.col_model)
        row_logits = self._batched_score(query, row_reps, self.row_model)
        cells = self._top_k_cells(row_logits, col_logits)
        return [
            {"row_ndx": ri, "col_ndx": ci, "confidence_score": s, "text": rows[ri][ci]}
            for ri, ci, s in cells
        ]
