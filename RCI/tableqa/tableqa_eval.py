...
def evaluate_pred_map(
    gt_file, qid2pred, cutoffs, *, answer_in_header=True, ignore_target_column=False
):
    ...
    print(
        f"MRR cell = {correctness[0, 0]}, column = {correctness[0, 1]}, row = {correctness[0, 2]}"
    )
    for ci, cutoff in enumerate(cutoffs):
        print(
            f"Hit@{cutoff} cell = {correctness[ci+1, 0]}, "
