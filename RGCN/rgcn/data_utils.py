    ...
    if (
        ...
        with RDFReader(graph_file) as reader:
            ...
            for i, rel in enumerate(relations if limit < 0 else relations[:limit]):
                ...
                print(
                    "Creating adjacency matrix for relation {}: {}, frequency {}".format(
        ...
        nodes_u_dict = {
            np.str_(to_unicode(key)): val for key, val in nodes_dict.items()
        }
        ...
        for nod, lab in zip(
            ...
            nod = np.str_(to_unicode(nod))  # type: unicode
